...
- **Round 6 (LLM provider abstraction + go free-tier, 2026-06-15): 7 NEW items, all TODO.** Plan:
  ...
  **Progress: 3/7 DONE (R6-1, R6-2, R6-3) · 0 IN-PROGRESS · 4 TODO (R6-4…R6-7). Last resume point: R6-4.**
...
- **R6-3 DONE** — shared rate limiter; behavior-preserving for Anthropic.
  ...
  - Commit: `pending` (backfill in R6-4's tracker update).
- RESUME AT: **R6-4** (Gemini adapter `lib/llm/gemini.ts` via `@google/genai`: `generateStructured` →
  `responseMimeType:'application/json' + responseSchema` + `JSON.parse`; `generateText` →
  `generateContent`; `system → systemInstruction`; preserve fence/notice + post-parse validation; add
  `GEMINI_API_KEY` env. Still default-Anthropic until R6-5).
