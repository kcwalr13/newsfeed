...
- **Round 6 (LLM provider abstraction + go free-tier, 2026-06-15): 7 NEW items, all TODO.** Plan:
  ...
  **Progress: 5/7 DONE (R6-1…R6-5; R6-5 code done, live acceptance pending Kyle's Vercel flip) · 0 IN-PROGRESS · 2 TODO (R6-6 docs, R6-7 optional batching). Last resume point: R6-6.**
...
- **⚠️ KYLE ACTION (after this deploys):** set `LLM_PROVIDER=gemini` in Vercel (`GEMINI_API_KEY` already
...
- RESUME AT: **R6-6** (Docs + ops: `.env.example` is done; add README + ARCHITECTURE entries for
  `LLM_PROVIDER`/`GEMINI_API_KEY`, the free-tier training-data caveat, the taste-model recalibration note
  [Haiku-scored vs Gemini-scored mixed space], the degraded-mode/deadline behavior, and the per-instance
  limiter caveat. The "create a key" Kyle-action is already done per the kickoff.).
