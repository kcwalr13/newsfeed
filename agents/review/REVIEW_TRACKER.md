...
- **Round 6 (LLM provider abstraction + go free-tier, 2026-06-15): 7 NEW items, all TODO.** Plan:
  ...
  **Progress: 2/7 DONE (R6-1, R6-2) · 0 IN-PROGRESS · 5 TODO (R6-3…R6-7). Last resume point: R6-3.**
...
- **R6-2 DONE** — Anthropic adapter + all 7 sites refactored to the interface; behavior-preserving.
  ...
  - Commit: `pending` (backfill in R6-3's tracker update).
- RESUME AT: **R6-3** (shared token-bucket rate limiter; wire every call through it incl. the unbounded
  curator-note fan-out; default Anthropic to unlimited so behavior stays unchanged until R6-5).
