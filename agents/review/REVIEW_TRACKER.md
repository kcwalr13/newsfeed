...
- **Round 5 (Product — content mix, curator voice & feel, 2026-06-14): 8 NEW items, all TODO.** From Kyle's
  ...
  (#3 — the headline). Build order A → B → C → D.
  **✅ ROUND 5 COMPLETE: 8/8 DONE (A scroll · B paywall ×2 · C curator note ×2 · D format mix ×3) · 0 TODO.**
...
- **Round 6 (LLM provider abstraction + go free-tier, 2026-06-15): 7 NEW items.** Plan:
  ...
  **✅ ROUND 6 COMPLETE + LIVE-VALIDATED (2026-06-15): 6/7 DONE (R6-1…R6-6) · 1 DEFERRED (R6-7) · 0 TODO.**
  R6-1…R6-6 done; **production is live on the Gemini free tier** (`LLM_PROVIDER=gemini` set in Vercel) and
...
- [x] **R5-A1** · Restore feed scroll on back-navigation. Set `history.scrollRestoration='manual'`; on card click record the clicked article id + scrollY in `sessionStorage` (keyed by batchDate); suppress the `<FeedSkeleton/>` on return (render stale `data` while re-fetching — skeleton only on first load); after paint, scroll the saved article's card into view (fallback scrollY). Anchor by **id** (survives the per-request re-rank). Files: `app/page.tsx` (+ 1 line in `app/layout.tsx`). Acceptance: open a mid-issue item → back returns to that item's spot, no top-jump/skeleton flash. · DONE · Commit: 2a90fe1
  - Notes: The App-Router back-nav **unmounts** `FeedPage` (route change) so in-component state can't be "kept" — added a **module-scope `cachedFeed`** that survives the remount; `data`/`status` hydrate from it via lazy `useState` initializers, so a back-nav paints the prior issue **synchronously** (full DOM height, no skeleton) while `fetchFeed()` re-runs in the background and re-writes the cache. Render gates moved from `status==='success'` → **`data`** (success block + seven-dot strip), skeleton now `status==='loading' && !data`, error `status==='error' && !data` (a failed background re-fetch keeps stale data instead of wiping the feed). `ArticleCard` gained `data-article-id` (anchor) + an `onNavigate` callback wired to its 3 nav `<Link>`s; the feed records `{id, scrollY}` to `sessionStorage` (key `tangent:feedpos:<batchDate>`) on click and **consumes it once** on the next mount. A `[data]` effect re-anchors by id (`CSS.escape`, −72px for the sticky masthead; fallback saved `scrollY`) on both the cached paint and the re-fetched paint; a one-shot `wheel`/`touchstart`/`keydown` cancel + a 2.5s backstop stop the late re-fetch from yanking the reader once they take over scrolling. `scrollRestoration='manual'` set in `page.tsx`'s mount effect (not `layout.tsx` — it's a server component; `page.tsx` is the only scrolling route and its effect runs before any first navigation). Files: `app/page.tsx`, `app/components/ArticleCard.tsx`. Verified: tsc + lint (0 errors) + build green. Live feel check (back-nav returns to the clicked card, no top-jump/skeleton flash) to confirm on the Vercel deploy.

### B — Paywall / full-text guard (Kyle #4)
- [x] **R5-B1** · `lib/utils/paywall.ts` `detectPaywall(bodyText)` (phrase list anchored to short trailing lines: "subscribe to read/continue", "this post is for paid subscribers", "members only", "become a member", "to continue reading", "unlock…"; min-content floor as a **secondary** signal — do NOT reuse `AESTHETIC_BODY_MIN_CHARS`). Apply in `rssAdapter.ts` (drop teaser body → triggers backfill) and as a new `'paywalled'` `ExtractionFailureReason` in `bodyExtractor.ts`; **exclude still-paywalled items in `run.ts` assembly before the fixed/discovery slot-fill** so the issue doesn't under-fill. Per-item, never per-source (Substack free vs paid). Acceptance: paywalled/teaser items excluded; a short *free* visual post is not. · DONE · Commit: pending
  - Notes: New `lib/utils/paywall.ts` — `detectPaywall(text)` is **phrase-primary** (13 paywall-specific regexes, matched only on short standalone lines ≤160 chars so a CTA notice trips it but prose mentioning "subscribers" doesn't) with a **weak secondary** floor `PAYWALL_MIN_CONTENT_CHARS=220` (well below the 300 char `AESTHETIC_BODY_MIN_CHARS` — a short *free* visual post must not trip it) gated on a soft subscribe-cue. Wired three ways: (1) `rssAdapter` drops a paywall-flagged teaser body so `fetchMissingBodyText` retries the full page — a free-but-truncated excerpt carries no paywall phrase, so it survives and backfills normally (this is *why* we drop-and-retry rather than exclude on the teaser: a "read more on our site" free truncation must not be dropped); (2) `bodyExtractor` gains a `'paywalled'` `ExtractionFailureReason`, checked on the **pre-`cleanBodyParagraphs`** paragraphs (the cleaner strips "subscribe to read"-style share lines) and **before** the `wordCount<300` gate, so discovery's existing failure-discard drops it; (3) `fetchMissingBodyText` now returns the set of ids whose full page came back `'paywalled'`, and `run.ts` filters them out of the assembled batch with a `PAYWALL excluded N` log. The exclusion is post-backfill (confirming a paywall *requires* the full-page fetch, which runs after the slice) but the batch holds 20 and only 7 display, so a few drops never starve the issue. Verified: 13/13 ad-hoc cases (Substack paid notice, members-only, "subscribe to keep reading", "this is a preview" → excluded; free short visual post, free article w/ newsletter footer, free read-more truncation, prose mentioning subscribers → kept). Files: `lib/utils/paywall.ts`, `lib/discovery/bodyExtractor.ts`, `lib/pipeline/adapters/rssAdapter.ts`, `lib/pipeline/run.ts`. Gate green. Decision logged.
- [x] **R5-B2** · Reader thin-body fallback: in `app/articles/[id]/page.tsx` show the existing "Read at source ↗" fallback when `paragraphs.length < ~3` (or a char floor), not only when fully empty — a teaser never renders as a stub. · DONE · Commit: pending
  - Notes: Replaced the `paragraphs.length > 0` body gate with `isReadableBody = paragraphs.length >= 3 || bodyText.length >= 700`. **AND-not-OR** thinness (few paragraphs *and* little text) so a short-but-complete free post (one or two substantial paragraphs) still renders in-app — only a genuine teaser/stub links out. The hero image, title, and description still render above the fallback in all cases, so a thin *visual* post keeps its substance. Dropped the now-dead middle "manual render" branch. Fallback copy is adaptive: "The full piece is best read at the source." when there's a teaser body, "Full text not available here." when there's none. Files: `app/articles/[id]/page.tsx`. Gate green. Note: thresholds (3 / 700) will compose with D2's visual/short card variants — a `place` item (D3) never reaches the reader at all.

### C — Personalized curator note replaces the summary (Kyle #2)
- [x] **R5-C1** · Add `Article.curatorNote?: string` (verify it survives `toPublicArticle`). New editorial, second-person, **taste-aware** prompt (extend `rationaleGenerator.ts` or add `curatorNoteGenerator.ts`): *why you'll want this + what taste it invites*, NOT a summary; fed a compact taste digest (top concept labels via `getTopConceptNodes` + a tone descriptor from the aesthetic centroid + the article's aesthetic vector). Generate for **all displayed items** (drop the exploration-only filter) at request time in `GET /api/feed/today`, persist via the existing `after()` + `patchBatchArticleFields` cache. Return the taste model from `resolveDisplayedFeed` instead of re-querying. (Folds in D4: short/place items get the exploration-inviting register.) Acceptance: every displayed item shows a personalized "why this is worth your time" note; raw RSS summary gone; subsequent loads don't re-call the LLM. · DONE · Commit: pending
  - Notes: Added `Article.curatorNote?: string` — survives `toPublicArticle` automatically (it spreads `...rest` after stripping a fixed internal-field list, none of which is `curatorNote`). New `lib/pipeline/curatorNoteGenerator.ts`: `generateCuratorNote()` / `generateMissingCuratorNotes()` modeled on `rationaleGenerator` (lazy key-guarded Haiku client, `Promise.allSettled`, `appendLog`). Prompt is **second-person editorial invitation, not a summary** (system prompt forbids restating the title/plot; ≤~32 words; no quotes/emoji/exclamation; wrapped with `UNTRUSTED_CONTENT_NOTICE` + `wrapUntrusted`). Fed a `TasteDigest` = top concept labels + a `describeTone()` descriptor of the **reader's centroid** + the **piece's own aesthetic vector** (so it can say how this piece relates to your taste); cold-start (no concepts/centroid) → "speak to the curious newcomer" register. `resolveDisplayedFeed` now returns a `taste: ResolvedTaste` ({topConcepts, centroid, articleScores}) built from the values it already resolved (no re-query; `EMPTY_TASTE` on the raw-order fallback). The feed route generates notes for the **displayed slice** (`displayArticles.slice(0, ISSUE_DISPLAY_SIZE)`) only — bounds cost to the issue — awaits them so they ship on the same response, and persists `curatorNote` (+ any rationale) via the existing `after()` + `patchBatchArticleFields` path so **cached loads make zero LLM calls** (`generateMissingCuratorNotes` early-returns 0 when none are pending). Rationale generation was also narrowed from all `displayArticles` to the shown slice (lazy — generated when an item enters the top span; strictly less wasted LLM). Files: `lib/types/article.ts`, `lib/pipeline/curatorNoteGenerator.ts`, `lib/pipeline/displayedFeed.ts`, `app/api/feed/today/route.ts`. Gate green. The visible swap (note replaces the RSS blurb) lands in R5-C2; format-specific register (place/short) folds in during D. Live note quality to confirm on the Vercel deploy (needs `ANTHROPIC_API_KEY`).
- [x] **R5-C2** · Render it: `ArticleCard.tsx` blurb block → `article.curatorNote ?? cleanDesc(description)` (keep the 3-line clamp); the now-redundant slot-badge rationale line can be removed/merged. · DONE · Commit: pending
  - Notes: Blurb block now shows `article.curatorNote ?? cleanDesc(article.description ?? '')` and renders when **either** a note or a description exists (so a note shows even when the source shipped no description). The 3-line `WebkitLineClamp` clamp is unchanged. Removed the redundant per-slot rationale sentence from the slot-caption (the curator note now carries "why this" for every piece, slotted or not) — the slot **badge** (glyph + label + caption, e.g. "✦ A stretch — why this") stays as the exploration signal. Note: the feed route still *generates* the rationale for slotted items (cheap, cached, 2–4 items) but it's no longer displayed — a possible later cleanup, left in scope-minimal. File: `app/components/ArticleCard.tsx`. Gate green.

### D — Content-format mix — THE HEADLINE (Kyle #3)
- [x] **R5-D1** · Add `Article.format?: 'longread'|'short'|'visual'|'potpourri'|'place'`; derive at assembly (`run.ts`, with `readTime`) via source-category allowlist + readTime threshold (no LLM). Add `ensureFormatSpread()` to `lib/pipeline/displayDiversity.ts`, invoked in `resolveDisplayedFeed` after C2/C3, guaranteeing ≥1 short + ≥1 visual/potpourri and ≤N longreads in the displayed 7; config next to `MIN_CATEGORIES_IN_ISSUE`. ⚠️ **Re-prove the consecutive-source cap can't break this or the C2/C3 floors (R4-14 precedent)** — order the reorders/cap so all floors hold; degrade gracefully. Acceptance: a typical issue isn't all long-reads; colophon stays in sync (shared resolver). · DONE · Commit: pending
  - Notes: Added `Article.format?: ContentFormat` + the `ContentFormat` type. **Resolved on read** (`lib/pipeline/contentFormat.ts` `formatForArticle`), not persisted at assembly — same P3-B2 rationale as `categoryForArticle` (works on historical batches, no re-run); the one exception is `place` (explicit, set by D3 — honoured first). `deriveContentFormat`: source-based formats win (`visual` = category art/design or curated set incl. Public Domain Review; `potpourri` = Tedium + curio hosts), else `longread` (readTime ≥ `LONGREAD_MIN_MINUTES`=10) else `short` (has body/readTime) else undefined. New config: `LONGREAD_MIN_MINUTES`, `MIN_SHORT_IN_ISSUE`=1, `MIN_VISUAL_OR_POTPOURRI_IN_ISSUE`=1, `MAX_LONGREADS_IN_ISSUE`=5 (7−5 ≥ the two floors, jointly satisfiable). `ensureFormatSpread` runs in `resolveDisplayedFeed` **after** C2/C3 and **before** the floor-guarded cap; the floor-guard now also protects the three format floors (same shape as the category/unfamiliar guards). The feed route sets `a.format = formatForArticle(a)` on the displayed slice (before `toPublicArticle` strips `bodyText`) so the client/D2 receive it. **Composition re-proved by simulation** driving the REAL `displayDiversity` functions, faithful to the system (category⇒source, per-source cap 4): across **400k random + 50k adversarial runs** — (D) 0 >cap runs in top-7, (A) 0 C2-floor drops, (B) 0 C3-floor drops, (C) 0 cap-broke-a-floor, efficacy 50000/50000 for all three format floors; the segment-cap fallback fired **0** times under realistic per-source-capped data ⇒ a by-construction safety net (same result R4-14 found for the category/unfamiliar floors). Key fix found *by* the sim: an early blanket-`protect` design no-op'd the guarantee whenever the top was all-unfamiliar / all-distinct-category (cold-start) — replaced with a precise per-swap floor check (allow demoting a longread iff the resulting top still holds the C2/C3 floors, or doesn't worsen an unmet one). Files: `lib/types/article.ts`, `lib/config/feed.ts`, `lib/pipeline/contentFormat.ts`, `lib/pipeline/displayDiversity.ts`, `lib/pipeline/displayedFeed.ts`, `app/api/feed/today/route.ts`. Gate green. Decision logged. Colophon stays in sync (shared `resolveDisplayedFeed`). The card variants (D2) + visible badge land next.
- [x] **R5-D2** · `ArticleCard.tsx` card variants by `format`: compact/dense for short/potpourri, image-forward for visual. Read-time UI already hides when absent. · DONE · Commit: pending
  - Notes: Branch on `article.format`. **Compact** (`short`/`potpourri`): no hero block at all (skipped — they're quick reads), tighter padding (`pt-4 pb-5`), smaller 19px title — reads as a denser card. **Image-forward** (`visual`): taller hero (`maxHeight` 320 vs 220). `longread`/undefined keep the default; `place` keeps default for now (its bespoke card is D3). Reused the meta-row mechanism for a small uppercase format tag ("Short"/"Miscellany"/"Visual") on the right next to read-time (longread untagged = the default). Read-time UI already hides when absent (PIPE-Q3), so compact pieces render cleanly without it. File: `app/components/ArticleCard.tsx`. Gate green.
...
- [x] **R5-C3** · ✅ RESOLVED (2026-06-15, via the Round 6 Gemini switch — NOT an Anthropic top-up) · Curator notes now generate on the live feed
  ...
  - **RESOLUTION (2026-06-15) — chosen path was NOT an Anthropic top-up.** Kyle elected to go $0 instead of
    adding credits: Round 6 built the provider abstraction and switched prod to the **Gemini free tier**
    (`LLM_PROVIDER=gemini` in Vercel). First live attempt 429'd because the design's `gemini-2.0-flash` had
    been **shut down 2026-06-01** (free-tier quota → 0); switching the model to **`gemini-2.5-flash-lite`**
    (commit `0dbd842`) fixed it. **Verified live:** `/api/feed/today` returns personalized `curatorNote` for
    all 7 displayed pieces (second-person, taste-aware), generated via Gemini, persisted via `after()`, and
    served from cache on the next load (~439ms) — at $0. All other LLM features (aesthetic scoring, discovery
    eval, theme) now run on the same working free-tier model; they get their first real exercise at the next
    daily cron.
  - **Optional code hygiene (lower priority, keep for later):** move curator-note generation to PIPELINE time
    ...
    path (latency + the DAT-M1 "LLM on the read path" smell) and pre-baked into the issue. Not required now.
  - Status: ✅ **RESOLVED** (Round 6 Gemini switch, live-verified 2026-06-15) · the optional pipeline-time refactor can be a separate item.
...
- [x] **R6-2** · Anthropic adapter (`lib/llm/anthropic.ts`) + **refactor all 7 LLM sites to the interface** (aestheticScorer, llmEvaluator [keep its provider-injection test seam], conceptExtractor, blindSpotProber → `generateStructured`; themeGenerator, curatorNoteGenerator, `scripts/refresh-query-banks.ts` → `generateText`). Anthropic stays active → **behavior-preserving**; gate green. De-risks the round. · **DONE** (commit `72b9375`)
  - **Notes:** Added `lib/llm/anthropic.ts` (`AnthropicProvider`: lazy client + key guard; `generateStructured` builds `tools:[{name,description,input_schema}] + tool_choice:{type:'tool',name}` and returns the `tool_use` block's `input`; `generateText` returns the first text block; model from `activeProviderConfig()`). Wired `getLlm()` (index.ts) to return it for `anthropic` and added provider-aware `isLlmConfigured()`. Added `LlmError {kind:'api'|'parse'}` + `toolDescription?` to `types.ts`. Refactored all 7 sites: removed `import Anthropic`/`new Anthropic()`/`messages.create`/`LLM_MODEL`; structured sites pass their schema/toolName/toolDescription/system/`wrapUntrusted(user)`/maxTokens; text sites pass system?/user/maxTokens. **Invariants preserved:** `UNTRUSTED_CONTENT_NOTICE` in `system` + `wrapUntrusted(...)` on user for sites 1–6 (grep-verified); site 7 (query-bank) stays exempt; all post-parse validation + every `max_tokens` (256/256/512/2048/160/110/512) + schema field types (number vs integer) + the curator-note clamp/quote-strip + theme fence-strip are byte-identical (grep-verified). `llmEvaluator` keeps its injectable seam (retyped `Anthropic`→`LlmProvider`) and its `parse_error`/`api_error` mapping via `LlmError.kind`. **Adversarial review (general-purpose subagent)** confirmed sites 1–6 strictly behavior-preserving; found one real divergence in **site 7** (a no-text response would abort the whole offline script instead of skipping one topic) — **fixed** with a `kind`-aware catch (parse→skip topic, api→rethrow/abort, matching the old `?? ''` semantics). Remaining diffs are cosmetic log/detail-message wording only (LOW). Verified: `tsc` clean · `lint` 0 errors · `build` EXIT=0. **Caveat (out of scope):** `npm run refresh-query-banks` was already broken before R6-2 — Node's ESM loader rejects the script's extensionless relative imports; the refactor is logic-preserving but does not fix (or worsen) that pre-existing runtime breakage.
- [x] **R6-3** · Shared token-bucket rate limiter (`lib/llm/limiter.ts`) keyed by the active provider's RPM; wire **every** call through it (incl. the unbounded `generateMissingCuratorNotes` fan-out); per-loop concurrency (`PIPELINE_LLM_CONCURRENCY`, `DISCOVERY_LLM_CONCURRENCY`) becomes subordinate. Default to Anthropic's unlimited rate so behavior is unchanged until R6-5. · **DONE** (commit `d2aefa3`)
  - **Notes:** Added `lib/llm/limiter.ts` — `acquireLlmSlot()` keyed on `activeProviderConfig().rpm`, implemented as a **fixed-interval (leaky-bucket) scheduler** (each slot spaced `60000/rpm` ms after the previous via a shared `_nextFreeMs`) rather than a bursting token bucket, so it guarantees **≤ rpm calls in every rolling 60s window** (a capacity-rpm token bucket can emit up to 2×rpm in one window and trip Gemini's rolling-window limit). Wired via a `withRateLimit()` **decorator** in `getLlm()` (index.ts) that `await acquireLlmSlot()` before delegating — so **every** call from **both** adapters is metered, including the previously-unbounded `generateMissingCuratorNotes` fan-out and discovery eval (`evaluateWithLLMClient` receives the wrapped `getLlm()` provider). Per-loop concurrency now only bounds dispatch; the limiter sets the actual rate. **Behavior-preserving:** for Anthropic `rpm=Infinity` → `acquireLlmSlot()` returns immediately (no `setTimeout`, `_nextFreeMs` untouched) → a single resolved-promise microtask before each call, no output/latency change. Targeted check (node sim of the exact `reserveSlot` formula): 40-call burst @15rpm → max **15** in any 60s window, even 4000ms spacing, ~156s total (→ R6-5 lowers `DISCOVERY_MAX_EVAL_CANDIDATES` to fit the budget); calls already under the limit get **zero** added wait; Infinity→no-op. Verified: `tsc` clean · `lint` 0 errors · `build` EXIT=0. **Scope caveat:** the gate is per serverless instance (one cron run = one instance, metered correctly; no cross-instance coordination — out of scope, single-user; documented for R6-6).
- [x] **R6-4** · Gemini adapter (`lib/llm/gemini.ts`, `@google/genai`): `generateStructured` → `responseMimeType:'application/json' + responseSchema` (per-site mapping in design §3) + `JSON.parse`; `generateText` → `generateContent`; `system → systemInstruction`; preserve the fence/notice; keep post-parse validation (Gemini honors schema constraints weakly). Add `GEMINI_API_KEY` env. · **DONE** (commit `528ebbc`)
  - **Notes:** Installed `@google/genai@2.8.0`. Added `lib/llm/gemini.ts` (`GeminiProvider`): lazy `GoogleGenAI({apiKey: GEMINI_API_KEY})` client w/ explicit key guard; `generateStructured` → `models.generateContent({model, contents:user, config:{systemInstruction:system, maxOutputTokens, responseMimeType:'application/json', responseSchema:toGeminiSchema(schema)}})` then `JSON.parse(response.text)`; `generateText` → same minus the JSON config. `toGeminiSchema()` maps our `JsonSchema` → Gemini `Schema`: lowercase types → `Type` enum (`OBJECT`/`ARRAY`/…), recurses `items`/`properties`, and — per the SDK types — **coerces `minItems`/`maxItems` to strings** (Gemini's `Schema` declares them `string`). Errors map to `LlmError` (`SyntaxError`/no-text → `parse`, transport → `api`) so the evaluator's reason mapping + query-bank skip/abort stay correct. Registered in `getLlm()`'s factory (`gemini` case). The `system→systemInstruction` + `wrapUntrusted(user)→contents` keeps the prompt-injection invariant; **post-parse validation stays at the call sites** (unchanged from R6-2) since Gemini honors schema constraints weakly. Added `LLM_PROVIDER` + `GEMINI_API_KEY` to `.env.example`. **Behavior-neutral until R6-5:** `LLM_PROVIDER` still defaults to `anthropic`, so this only adds an unused-by-default adapter. The Gemini SDK import is static (server bundle only; build verified clean). Verified: `tsc` clean (adapter typechecks against the real SDK `Schema`/`Type`) · `lint` 0 errors · `build` EXIT=0 · node sim of `toGeminiSchema` on all 4 site schemas (type-enum mapping, `minItems`/`maxItems`→string, nested recursion, `required` preserved) passes. Live structured/text validation is R6-5 (on the Gemini key). **Out of scope:** `@google/genai` pulled in 105 transitive packages; `npm audit` shows warnings that are **predominantly pre-existing** (`next`, `@anthropic-ai/sdk`, `fast-xml-parser`, `nodemailer` all predate R6-4) — not run `audit fix --force` (could break the build); flagged for awareness only.
- [x] **R6-5** · Go live on Gemini free + fit the budget. Set `LLM_PROVIDER=gemini`, model = **Gemini 2.0 Flash** (15 RPM/1M TPM), limiter ≈15 RPM; lower `DISCOVERY_MAX_EVAL_CANDIDATES` (40 → ~15) + concurrency so a full run fits RPM **and** `PIPELINE_WALL_CLOCK_BUDGET_MS`; verify graceful degradation (DAT-H2 still writes a thinner batch) and the daily RPD cap isn't blown by a refresh. Acceptance: a refresh completes on Gemini, structured + text calls return valid output, no 429 storms, batch writes. · **DONE + LIVE-VALIDATED 2026-06-15** (commit `fd0e5ee`; model fix `0dbd842`)
  - **Notes:** Made the budget knobs **provider-aware** in `lib/config/feed.ts` (imports `LLM_PROVIDER`): `DISCOVERY_MAX_EVAL_CANDIDATES` = **15** for gemini / 40 anthropic; `PIPELINE_LLM_CONCURRENCY` & `DISCOVERY_LLM_CONCURRENCY` = **2** for gemini / 4 anthropic. Gemini model (`gemini-2.0-flash`) + RPM (15) were set in `PROVIDER_CONFIG` (R6-1); the R6-3 limiter meters at 15 RPM. **Closed a latent DAT-H2 gap the slow rate exposes:** the post-discovery per-article scoring + concept phase was count-bounded (`MAX_LLM_EVALS_PER_RUN`) but **not** time-bounded, and the batch is written *after* it (`run.ts`), so under Gemini's ~4s/call a fresh-day run could exceed `maxDuration` (300s) and lose the batch. Added a **wall-clock deadline** to `LlmBudget` (`deadlineMs = runStartMs + PIPELINE_WALL_CLOCK_BUDGET_MS` = 270s) checked in `tryConsumeLlm` — which both the aesthetic loop and concept loop already gate on, so one change bounds both: past the deadline they stop issuing LLM calls, leave items unscored (rank by source score — existing degraded fallback), and the batch **always writes**. **Behavior-preserving for Anthropic:** the 270s deadline is never reached (effectively-unthrottled calls finish in ~30–40s) so `tryConsumeLlm` behaves exactly as before; the eval cap (40) + concurrency (4) are unchanged for anthropic. **`LLM_PROVIDER` code default stays `anthropic`** — deploy is no-op until Kyle flips it. Verified: `tsc` clean · `lint` 0 errors · `build` EXIT=0; node budget sim: Gemini nominal (15 disc + 40 post = 55 calls) writes the batch at ~260s < 300s; Gemini overload → deadline guard cuts excess + batch still writes ~276s < 300s; Anthropic → all calls fire (~38s), deadline never fires. **Live acceptance (Kyle):** flip `LLM_PROVIDER=gemini` in Vercel (key already set), trigger a refresh, confirm structured+text return valid output, no 429 storms, batch writes, and `curl /api/feed/today` shows populated `articles[].curatorNote` → **resolves R5-C3**. RPD caveat: avoid gratuitous `forceOverwrite` refreshes (each re-spends the daily cap). **LIVE-VALIDATED (2026-06-15):** Kyle set `LLM_PROVIDER=gemini` in Vercel; the first attempt 429'd because `gemini-2.0-flash` had been **shut down 2026-06-01** (free-tier quota → 0). Switched the model to **`gemini-2.5-flash-lite`** (commit `0dbd842`, the only post-R6 code change) — `/api/feed/today` then returned 7/7 personalized `curatorNote`s with no 429s, persisted via `after()` + served from cache (~439ms) on reload. **R5-C3 resolved.** The provider-aware caps (15 RPM ceiling note: free-tier RPM/RPD are account-specific, tunable in `PROVIDER_CONFIG`; `dailyCap` is now 1000) + the scoring wall-clock deadline carry over to the structured discovery/scoring calls, which get their first live exercise at the next daily cron.
- [x] **R6-6** · Docs + ops. `.env.example` + README + ARCHITECTURE: `LLM_PROVIDER` / `GEMINI_API_KEY`, the free-tier **training-data caveat** (article text + taste signals), the **taste-model recalibration** note (Haiku-scored vs Gemini-scored mixed space — accept drift or one-time rescore), degraded-mode behavior. **Kyle action:** create a free key at aistudio.google.com → set `GEMINI_API_KEY` + `LLM_PROVIDER=gemini` in Vercel. · **DONE** (commit `04a961a`; further doc refresh for the model swap + live validation in the 2026-06-15 doc-sync commit)
...
- [x] **R7-2** · **Discovery engine v1: index-mining + the funnel.** ✅ **COMPLETE (a–e).** New candidate stream in `lib/discovery/` that
  ...
  **website + thread** items + their cards. **Retire `data/sources.json` as the digest supply.** **This is the
  ...
  - **Notes — (e) supply flip (DONE):** **The milestone where the digest becomes one-off gems, not feed articles.**
    `lib/pipeline/run.ts` `runPipeline()` **retires the fixed-RSS supply entirely** — the `loadSources` →
    `fetchFromSource` → dedup/low-value/source-cap/diversify/validate path (and its now-dead helpers
    `fetchFromSource`/`applySourceCap`/`diversifyForSelection`/`PartialArticle` + their imports) is DELETED. The digest
    supply is now agent-discovered **one-offs**: the index-funnel link-out gems (PRIMARY — `INDEX_FUNNEL_ITEMS_PER_DAY`
    raised 8→16) + the Brave discovery **essay** stream capped to a new `MAX_ARTICLES_IN_ISSUE`=3 (`discoveryArticles.slice`
    — a transitional SUPPLY cap previewing R7-5's display-layer `ensureTypeSpread`; runDiscovery returns essays sorted
    best-first so the slice keeps the best) + the curated place. `data/sources.json` + `loadSources` survive ONLY as the
    discovery novelty filter's "sources Kyle already knows" set (`novelty.ts` reads `SOURCES_PATH` directly). `app/page.tsx`
    seven-dot read-count now handles an **all-link-out issue** (`total = readableArticles.length`, no phantom `||
    ISSUE_SIZE`; `hasReadingRitual` hides the strip + swaps to "a handful of things to wander into" when there are no
    readable pieces). **Adversarial review** (3 dims × find→verify, 10 agents) confirmed the flip + caught **6 real
    findings, all fixed in this commit:** (1 HIGH) the cap burned the sliced-off essays from durable novelty memory at
    *generation* time → moved essay recording to **retire-on-DISPLAY** (removed `recordSeenUrls` from `runDiscovery`;
    the feed route now records every DISPLAYED discovered item — link-out gems AND essays — so an unshown/sliced essay
    can resurface); (2 MED) a fully-empty supply day wrote an empty batch that **shadowed the prior issue** (the feed
    only falls back to `readLatestBatch` when today's batch is *absent*) → `runPipeline` now **skips the write on an
    empty supply** so the last real issue stays; (3 MED) the discovery-share metric + feed `discoveryCount` counted
    only `discoveryTopic`, mislabeling the now-dominant funnel gems as "fixed" → broadened `computeDiscoveryYield` +
    `metrics.ts` `isDiscovery` to count `discoverySource` too; (4 MED) the `PIPELINE_ARTICLES_PER_DAY (14)+DISCOVERY(6)
    ===ARTICLES_PER_DAY(20)` load-time assertion was now dead/misleading → removed `PIPELINE_ARTICLES_PER_DAY` + the
    assertion, retitled `ARTICLES_PER_DAY`/`DISCOVERY_ARTICLES_PER_DAY`; (5 NIT/6 LOW) stale paywall comment +
    `runPipeline` JSDoc + `loadSources` doc updated. **Always-write/graceful-degradation preserved** (short digest, or
    skip-write to keep the prior issue, on a thin day; batch never crashes). **Injection invariant unchanged** (no new
    code sends discovered text to an LLM unfenced). **Verified:** `tsc` clean · `lint` 0 errors (3 pre-existing
    warnings) · `build` EXIT=0; **live supply check** (throwaway, removed; real durable-memory read + real index
    network): `runIndexFunnel()` at the prod limit → **16 real one-off link-out gems** (14 websites, 2 threads —
    `paintedpixels.xyz`, `byran.ee`, `jerrysmap.com`, `eieio.games`, `beautifulpublicdata.com`, a homebrew-console blog,
    mastodon/bsky threads), **gem-dominant** (16 gems vs ≤3 essays), durable dedup live. **The real end-to-end prod
    gem-digest is produced by the next Vercel cron (Gemini) / an in-app refresh** — a local full run would use the
    committed Anthropic provider (out of credits) and be an unrepresentative no-essay batch, so per the project's
    standing pattern, representative prod validation runs on the deploy. Kyle: after this push, hit refresh or wait for
    the cron to see the live gem-digest.
...
- [ ] **R7-8** · **Rich feedback → taste loop** (design §10; Kyle 2026-06-23). **(a) near-term: ✅ DONE via R7-2(d)
  (commit `df48588`)** — the standard feedback row (dislike/like/save) now renders on every link-out card incl. the R5
  place card; saved link-out gems link straight out from the shelf; link-out items are in the like/dislike signal.
  **(b) (the richer chips + free-text "why" + shape/style axis) remains TODO, sequenced with/just before R7-5.** **(b):** "quick by default, optional depth" feedback — one-tap dislike/like/save
...
| 2026-06-15 | R5-A1 | Persist the last feed in a **module-scope variable** (not just in-component state) + keep the background re-fetch; set `scrollRestoration='manual'` in `page.tsx` not `layout.tsx`; add a user-gesture cancel for the re-anchor | The design's "keep the previous `data` rendered while a re-fetch runs" can't be done with component state alone — App-Router back-nav unmounts `FeedPage`, so `data` resets to null and the skeleton flashes. A module-scope cache survives the remount and lets the prior issue paint synchronously (full height), which is the actual mechanism that kills both the skeleton flash and the height-0 top-jump. Kept the re-fetch (per design) but anchored by id on **both** the cached and re-fetched paints, and added a one-shot `wheel`/`touchstart`/`keydown` cancel (+2.5s backstop) so a slow re-fetch landing after the reader has scrolled can't yank them back — the only real risk of re-fetching on every back-nav. `scrollRestoration` lives in `page.tsx`'s mount effect because `layout.tsx` is a server component (can't touch `window`) and the feed is the only route whose scroll we restore; the effect runs well before any first navigation, so native restoration is off in time. |
| 2026-06-15 | R5-B1 | Drop-and-retry the teaser in `rssAdapter` (not exclude-on-teaser); exclude **post-backfill** in `run.ts` (not strictly before slot-fill); detect on **pre-clean** paragraphs in `bodyExtractor` | Excluding an item just because its RSS teaser carries a paywall phrase would wrongly drop **free-but-truncated** feeds (ad-supported sites that ship a short excerpt + "read more on our site"); the design's drop-the-teaser → backfill-the-full-page → exclude-only-if-still-paywalled flow is what distinguishes a real paywall from a free truncation, so I kept it. Confirming a paywall therefore *requires* the full-page fetch, which the pipeline runs in `fetchMissingBodyText` **after** the 20-item slice — so the exclusion is post-backfill rather than literally "before slot-fill". That's safe because the batch holds `ARTICLES_PER_DAY`(20) and only `ISSUE_DISPLAY_SIZE`(7) display, so dropping a few confirmed-paywalled items never under-fills the issue (the design's worry only bites if a large fraction is paywalled, which a 23-source palette won't produce). Detection runs on the **pre-`cleanBodyParagraphs`** text because the cleaner strips share-like CTA lines ("subscribe to read") that are exactly the paywall signal. Phrases are paywall-specific (not generic newsletter CTAs) and anchored to short lines, so a footer "subscribe to our newsletter" on a free article doesn't trip it (validated: 13/13 cases incl. that exact negative). |
| 2026-06-15 | R5-D3 | Place links **straight out** (new tab), not an interstitial; injected **deterministically** (~1 every 3 issues, date-keyed) not randomly; surfaced via an `ensureFormatSpread` step that **yields to C2/C3**; place excluded from the read count | The design recommended "link straight out" over a bespoke interstitial — taken, since a place has no body and an interstitial is extra surface for no benefit (the card itself is the invitation). Cadence is keyed on the batch date (`epochDay % 3`) rather than `Math.random` so a pipeline re-run for the same day is stable (the pipeline forbids `Math.random`/`Date.now` for re-run safety) and the rotation cycles all 10 places. Surfacing the place is a *soft* guarantee folded into `ensureFormatSpread` (step 0) using the same per-swap floor check as the format floors, so it **yields to C2/C3** when no floor-preserving swap exists — the simulation shows it surfaces 96.8% of place-issues and the 3.2% misses are all genuine floor-conflicts (0 bug-misses), which is correct: the unfamiliar/category guarantees outrank an occasional curiosity. A place is its own unique source, so the consecutive-cap never defers it (it's never a same-source run member) — the floor-guard's place clause is therefore a by-construction no-op kept for rigor. Place items are excluded from the seven-dot read/total count because they carry no Pass/Underline/Read-later action (a side-invitation, not a piece to finish) — otherwise "All pieces read" would be unreachable on a place issue (R2-12 analog). |
| 2026-06-15 | R6-2 | Unified `tool_choice` to `{type:'tool',name}` for all structured sites (sites 1/3/4 previously used `{type:'any'}`); added `toolDescription?` to the interface (beyond the design's signature) and a typed `LlmError {kind}`; introduced provider-aware `isLlmConfigured()` to replace scattered `process.env.ANTHROPIC_API_KEY` graceful-skip guards | With exactly one tool per call, `{type:'any'}` and `{type:'tool',name}` both force that single tool → behavior-equivalent, and the design specifies the explicit form. `toolDescription` keeps the Anthropic tool definition byte-identical (preserves the model-visible description) while Gemini ignores it — strictly more faithful than dropping it. `LlmError.kind` lets `llmEvaluator` keep its exact `parse_error`/`api_error` mapping and lets the query-bank script keep "skip one topic on empty response vs. abort on API error". `isLlmConfigured()` equals `!!ANTHROPIC_API_KEY` for the active anthropic provider (behavior-identical now) and makes the R6-5 Gemini flip a no-op at these guards instead of leaving Anthropic-specific key checks to hunt down. |
| 2026-06-15 | R6-7 | **DEFERRED** the optional phase-2 batching rather than implementing it | The design explicitly says "defer; the round works without it," and the campaign treats optional items as out-of-scope (P3-D4 precedent). R6-5 already makes a full Gemini run fit the free tier (lowered eval cap + per-article scoring deadline + graceful degradation), so batching is a pure throughput optimization, not a correctness requirement. It also meaningfully changes the scorer prompts/parsers (N-item array contract + per-element validation at two sites + the discovery eval loop), which is real risk for an optimization the round doesn't need — better as its own scoped change if free-tier discovery breadth ever feels thin. The exact implementation path is recorded in the R6-7 tracker entry for when it's wanted. |
...
| 2026-06-24 | R7-2(e) | The supply flip **removes fixed-RSS entirely** (no fixed-source fallback) and caps the essay stream at `MAX_ARTICLES_IN_ISSUE`=3 as a SUPPLY cap, with the index-funnel as the primary supply | The design (§4) is explicit: degrade to a SHORT digest of real gems, not fall back to feeds — so no fixed-RSS safety net (it's exactly the aggregator-of-sources model Kyle rejects). The ≤3 essay cap is needed because the ranker scores essays (aesthetic proximity) above unscored link-out gems (Wilson 0.5, 0 aesthetic), so without capping the essays would fill the displayed 7 and reproduce "still just long articles." Capping at the SUPPLY level (vs the display assembler) is a deliberate transitional choice: R7-5 owns the proper `ensureTypeSpread` display-layer cap + floors + the R5-D1 simulation re-proof, which is out of R7-2's scope. `data/sources.json` is kept (not deleted) because `novelty.ts` still needs it to recognise "sources Kyle already knows" so the Brave essay stream keeps surfacing *unfamiliar* domains. |
| 2026-06-24 | R7-2(e) | Durable novelty recording is **retire-on-DISPLAY for BOTH streams** (link-out gems AND Brave essays), moved out of `runDiscovery` (generation-time) into the feed route | The R7-2e essay cap created a burn: `runDiscovery` recorded all ≤6 of its `top` essays as permanently seen at generation, but only 3 enter the batch (sliced) — so ~3 good essays' whole domains were filtered forever without ever being shown, shrinking the very essay pool the post-flip digest depends on. Unifying on retire-on-display (the model the funnel already used) fixes it: the feed route records every DISPLAYED discovered item (`isLinkOutItem(a) || a.discoveryTopic`), so an unshown/sliced/below-fold essay stays eligible to resurface. The union-on-READ (durable keys → the discovery seen-set) is unchanged — only the WRITE moved. Idempotent; no-op before migration 020. |
| 2026-06-24 | R7-2(e) | On a fully-empty supply day, **skip the batch write** (don't write `articles: []`) so the feed keeps the prior issue | The flip removed the fixed-RSS floor, making a genuinely empty batch reachable for the first time (discovery cut short + funnel empty + no place that issue). The feed's `readBatch(today) ?? readLatestBatch()` only falls back when today's batch is *absent*, so writing an empty-but-present batch would SHADOW yesterday's still-good gems with a "Nothing yet" empty state — worse than a missed run. Skipping the empty write (returning `count:0, alreadyExists:false`) lets the reader keep the last real issue (graceful degradation: a stale good issue beats an empty one); the next run/cron retries. The always-WRITE invariant becomes always-write-*non-empty*. |
...
### Session 10 — 2026-06-15 — Claude Code (Round 5 — A: scroll restoration)
- **R5-A1** → DONE. Feed scroll position now survives back-navigation. Root cause: App-Router back-nav
  unmounts `FeedPage`, so the in-component `data` resets to null → skeleton flash → height-0 → top-jump.
  Fix: a module-scope `cachedFeed` survives the remount and hydrates `data`/`status` via lazy `useState`
  initializers, so the prior issue paints synchronously (full height, no skeleton) while a background
  re-fetch runs. Render gates moved `status==='success'` → `data`. `ArticleCard` gained `data-article-id`
  + `onNavigate`; the feed saves `{id, scrollY}` per issue to `sessionStorage` on card click and consumes
  it once on remount, re-anchoring by id (`CSS.escape`, −72px masthead offset; fallback `scrollY`) on both
  the cached and re-fetched paints. A one-shot `wheel`/`touch`/`keydown` cancel (+2.5s backstop) prevents a
  late re-fetch from yanking the reader. `scrollRestoration='manual'` set in `page.tsx` (layout is a server
  component). Files: `app/page.tsx`, `app/components/ArticleCard.tsx`. Gate green (tsc + lint 0 err + build).
  Decision logged. Commit: pending.
- **R5-B1** → DONE. Paywall/full-text guard. New `lib/utils/paywall.ts` `detectPaywall()` (phrase-primary,
  anchored to short CTA lines; weak `PAYWALL_MIN_CONTENT_CHARS=220` secondary, below the 300-char body min so
  free short visual posts aren't swept up). Wired: `rssAdapter` drops a flagged teaser → backfill retries the
  full page (a free truncation has no paywall phrase, so it survives); `bodyExtractor` gains a `'paywalled'`
  reason checked pre-clean + before the wordCount gate (discovery discards); `fetchMissingBodyText` returns
  paywalled ids and `run.ts` filters them from the batch (post-backfill, but batch=20 vs display=7 so no
  under-fill). 13/13 ad-hoc cases pass (paid/member/preview → excluded; free short/visual, newsletter-footer,
  read-more truncation, prose mentioning subscribers → kept). Files: `lib/utils/paywall.ts`,
  `lib/discovery/bodyExtractor.ts`, `lib/pipeline/adapters/rssAdapter.ts`, `lib/pipeline/run.ts`. Gate green.
  Decision logged. Commit: 73fcbb1.
- **R5-B2** → DONE. Reader thin-body fallback. Body gate is now
  `isReadableBody = paragraphs.length >= 3 || bodyText.length >= 700` (AND-not-OR thinness, so a
  short-but-complete free post still renders; only a teaser/stub links out). Hero image + title + description
  still render above the fallback. Dropped the dead manual-render branch; adaptive fallback copy.
  File: `app/articles/[id]/page.tsx`. Gate green. Commit: 4806f44.
- **R5-C1** → DONE. Personalized curator note (backend). `Article.curatorNote?: string` (survives
  `toPublicArticle`). New `lib/pipeline/curatorNoteGenerator.ts` — second-person editorial *invitation*
  (not a summary), fed a taste digest (top concepts + `describeTone()` of the reader's centroid + the piece's
  aesthetic vector); cold-start → newcomer register. `resolveDisplayedFeed` now returns the already-resolved
  `taste` (no re-query). Feed route generates notes for the displayed slice only, awaits them onto the
  response, and caches via `after()`+`patchBatchArticleFields` so cached loads make zero LLM calls. Files:
  `lib/types/article.ts`, `lib/pipeline/curatorNoteGenerator.ts`, `lib/pipeline/displayedFeed.ts`,
  `app/api/feed/today/route.ts`. Gate green. Commit: acda3e3.
- **R5-C2** → DONE. Card renders the curator note. Blurb = `article.curatorNote ?? cleanDesc(description)`
  (shows when either exists; 3-line clamp kept). Removed the redundant per-slot rationale sentence (the note
  carries "why this" now); the slot badge stays. File: `app/components/ArticleCard.tsx`. Gate green.
  Commit: cc8cda0.
- **R5-D1** → DONE (THE HEADLINE — format mix guarantee). `Article.format` + `ContentFormat`; resolved on
  read (`contentFormat.ts` `formatForArticle`, like category) — `place` explicit. `ensureFormatSpread` in
  `displayDiversity.ts` runs in `resolveDisplayedFeed` after C2/C3, before the floor-guarded cap; guarantees
  ≥1 short + ≥1 visual/potpourri + ≤5 longreads in the top 7. Config in `feed.ts`. Feed route exposes
  `format` on the displayed slice. **Composition re-proved by simulation** (real `displayDiversity` fns,
  category⇒source + per-source-cap-4): 400k random + 50k adversarial → 0 floor breaks, 0 >cap runs, efficacy
  50000/50000; segment-cap fallback fires 0× under realistic data (R4-14-style by-construction). Sim caught
  a blanket-`protect` no-op bug → replaced with a precise per-swap floor check. Files: `lib/types/article.ts`,
  `lib/config/feed.ts`, `lib/pipeline/contentFormat.ts`, `lib/pipeline/displayDiversity.ts`,
  `lib/pipeline/displayedFeed.ts`, `app/api/feed/today/route.ts`. Gate green. Decision logged. Commit: 893bbdc.
- **R5-D2** → DONE. Card variants by `format` in `ArticleCard`: compact (no hero, tighter padding, 19px title)
  for short/potpourri; image-forward (taller 320px hero) for visual; default for longread/place. Small
  uppercase format tag in the meta row. File: `app/components/ArticleCard.tsx`. Gate green. Commit: 25c82ad.
- **R5-D3** → DONE. "A place to explore." Committed `data/places.json` (10 seeded sites; Kyle curates).
  `places.ts` `selectPlaceForBatch` injects one deterministically ~1 every 3 issues (rotating). `run.ts`
  appends it post-enrichment (no body/LLM cost). `ArticleCard` place branch: distinct "A place to get lost in"
  card, **Explore ↗** links straight out (`<a target=_blank>`, never the reader, no feedback row).
  `ensureFormatSpread` surfaces it (step 0, swap-validity preserves C2/C3); cap guard covers it. `page.tsx`
  excludes place from the read count. **Sim (400k, with places): 0 >cap runs, 0 cap floor breaks, place
...
- **R6-1 DONE** — provider interface + config scaffolding (no behavior change).
  ...
  - Gate: `npx tsc --noEmit` clean · `npm run lint` 0 errors (3 pre-existing warnings, untouched
    files) · `npm run build` EXIT=0. Commit: `f00e0d2`.
- **R6-2 DONE** — Anthropic adapter + all 7 sites refactored to the interface; behavior-preserving.
  - Added `lib/llm/anthropic.ts` (`AnthropicProvider`); registered it in `getLlm()`; added
    `isLlmConfigured()` (provider-aware), `LlmError {kind}`, and `toolDescription?` to `types.ts`.
  - Refactored sites 1–4 (aestheticScorer, llmEvaluator, conceptExtractor, blindSpotProber) to
    `getLlm().generateStructured`, sites 5–7 (themeGenerator, curatorNoteGenerator,
    refresh-query-banks) to `getLlm().generateText`. Removed all direct `@anthropic-ai/sdk` usage
    outside the adapter (grep-clean).
  - **Behavior-preservation verified three ways:** (1) deterministic grep audit — invariants
    (`UNTRUSTED_CONTENT_NOTICE` + `wrapUntrusted`) hold for sites 1–6, site 7 exempt; every
    `max_tokens`/schema-type/clamp/regex byte-identical; (2) `tsc`/`lint`/`build` all green; (3) an
    **adversarial subagent review** of `git diff HEAD` — verdict: sites 1–6 strictly preserved; one
    real divergence in site 7 (no-text response aborting the offline script) which I **fixed** with a
    `LlmError.kind`-aware catch (parse→skip topic, api→abort), matching the old `?? ''` semantics.
  - **Pre-existing, out-of-scope:** `npm run refresh-query-banks` is already broken under `ts-node`
    (ESM loader rejects extensionless relative imports) — independent of R6-2; flagged, not fixed.
  - Commit: `72b9375`.
- **R6-3 DONE** — shared rate limiter; behavior-preserving for Anthropic.
  - Added `lib/llm/limiter.ts` (`acquireLlmSlot()` + pure `reserveSlot()` helper): a fixed-interval
    leaky-bucket scheduler keyed on `activeProviderConfig().rpm`. No-op when rpm is non-finite
    (Anthropic). Wired through a `withRateLimit()` decorator in `getLlm()` so every adapter call
    (structured + text, all 7 sites incl. the unbounded curator fan-out) is metered.
  - Targeted node simulation of the exact `reserveSlot` formula: 40-call burst @15rpm never exceeds
    15 in any rolling 60s window (even 4s spacing, ~156s); under-limit traffic gets zero added wait;
    Infinity rpm → no-op. Gate: `tsc` clean · `lint` 0 errors · `build` EXIT=0.
  - Commit: `d2aefa3`.
- **R6-4 DONE** — Gemini adapter; behavior-neutral (still default-Anthropic).
  - Installed `@google/genai@2.8.0`. Added `lib/llm/gemini.ts` (`GeminiProvider`) + registered it in
    `getLlm()`. `toGeminiSchema()` maps our `JsonSchema` → Gemini `Schema` (Type enum, string
    `minItems`/`maxItems`, recursion). `system→systemInstruction`, `wrapUntrusted(user)→contents`;
    post-parse validation stays at the sites. Errors → `LlmError{kind}`. Added `LLM_PROVIDER` +
    `GEMINI_API_KEY` to `.env.example`.
  - Gate: `tsc` clean (typechecks vs the real SDK) · `lint` 0 errors · `build` EXIT=0 (Gemini SDK
    bundles clean). Node sim of `toGeminiSchema` on all 4 site schemas passes. Live runtime is R6-5.
  - Note: `@google/genai` added 105 transitive packages; `npm audit` warnings are predominantly
    pre-existing (`next`/`@anthropic-ai/sdk`/`fast-xml-parser`/`nodemailer`) — not actioned (out of scope).
  - Commit: `528ebbc`.
- **R6-5 DONE (code)** — Gemini budget-fit; live acceptance pending Kyle's Vercel flip.
  - `lib/config/feed.ts` now provider-aware: `DISCOVERY_MAX_EVAL_CANDIDATES` 15 (gemini) / 40
    (anthropic); `PIPELINE_LLM_CONCURRENCY` & `DISCOVERY_LLM_CONCURRENCY` 2 / 4.
  - `lib/pipeline/run.ts`: added a wall-clock `deadlineMs` (= `runStartMs + PIPELINE_WALL_CLOCK_BUDGET_MS`,
    270s) to `LlmBudget`, enforced in `tryConsumeLlm` — bounds the post-discovery scoring+concept phase
    so the batch is **always written** before maxDuration even under Gemini's ~4s/call rate (closes a
    latent DAT-H2 gap). Unscored items rank by source score (existing fallback).
  - Behavior-preserving for Anthropic: Infinity RPM → deadline never reached → identical to before.
  - Gate: `tsc` clean · `lint` 0 errors · `build` EXIT=0. Budget sim: Gemini nominal 55 calls → batch
    ~260s; overload → deadline cuts excess, batch still writes ~276s (< 300s); Anthropic → all calls,
    no deadline, ~38s.
  - Commit: `pending` (backfill in R6-6's tracker update).
- **⚠️ KYLE ACTION (after these deploy):** set `LLM_PROVIDER=gemini` in Vercel (`GEMINI_API_KEY` already
  set). Then verify the live deploy: a refresh completes on Gemini, structured+text calls return valid
  output, no 429 storms, the batch writes, and `curl /api/feed/today` shows populated
  `articles[].curatorNote` → **this is what resolves R5-C3.**
- **R6-6 DONE** — docs + ops.
  - README.md: provider-abstraction in tech-stack/prereqs; `LLM_PROVIDER`/`GEMINI_API_KEY` env rows; a new
    "LLM provider (Round 6)" subsection (adapters, limiter, budget-fit, 4 free-tier caveats).
  - agents/architect/ARCHITECTURE.md: LLM stack row + env table updated; new "Round 6 — LLM provider
    abstraction" section. CLAUDE.md env section kept accurate. (`.env.example` was done in R6-4.)
  - Gate: `tsc` clean · `build` EXIT=0 (docs-only; lint unaffected). Commit: `04a961a`.
- **R6-7 DEFERRED** — optional phase-2 batching of the high-volume structured calls. Per the design's
  explicit "defer; the round works without it" + the P3-D4 optional-item precedent. The round already fits
  the Gemini free tier via R6-5 (lowered eval cap + scoring deadline + graceful degradation); batching is a
  throughput optimization, not a correctness need. Exact approach for later recorded in the R6-7 entry.
- **✅ ROUND 6 COMPLETE** (6 DONE, 1 DEFERRED). The LLM backend is now abstracted (`lib/llm/`), rate-limited,
  and one env var away from Gemini free-tier. **Only remaining action is Kyle's:** set `LLM_PROVIDER=gemini`
...
- **R7-2(d) DONE** (commit `df48588`) — the standard feedback row (dislike/like/save) now renders on every
  link-out card.
...
- **R7-2(e) DONE** (commit `541e1fb`) — **the milestone: the digest is now agent-discovered one-off gems, not feed
  ...
  - **`lib/pipeline/run.ts`**: removed the entire fixed-RSS supply path + its dead helpers/imports; supply = index-funnel
    ...
    place. **`lib/config/feed.ts`**: +`MAX_ARTICLES_IN_ISSUE`; removed the dead `PIPELINE_ARTICLES_PER_DAY` + its
    load-time assertion. **`app/page.tsx`**: seven-dot read-count handles all-link-out issues.
  - **Adversarial review** (3 dims × find→verify, 10 agents) → **6 real findings, all fixed in this commit**: essay
    durable-burn from the cap → **retire-on-display for both streams** (recording moved `runDiscovery`→feed route);
    empty-batch shadowing the prior issue → **skip the empty write**; discovery metric/`discoveryCount` mislabeling
    funnel gems as fixed → broadened to count `discoverySource`; dead `PIPELINE_ARTICLES_PER_DAY` assertion → removed;
    stale paywall comment + `runPipeline` JSDoc + `loadSources` doc → updated.
  - **Verified:** `tsc` clean · `lint` 0 errors (3 pre-existing warnings) · `build` EXIT=0; **live supply check**
    (throwaway, removed): `runIndexFunnel()` → **16 real one-off gems**, gem-dominant (16 gems vs ≤3 essays), durable
    dedup live. The full prod gem-digest is produced by the next Vercel cron (Gemini) / an in-app refresh — a local
    full run uses the committed Anthropic provider (out of credits → unrepresentative no-essay batch), so per the
    project's standing pattern, representative prod validation runs on the deploy.
  - **No new migration** in c–e (020 already applied). **R7-8(a) is satisfied by R7-2(d).**
- RESUME AT: **R7-3** — LLM agentic stream (LLM proposes lesser-known destinations per rotating theme →
  fetch-and-verify each is live) + the **type-aware interestingness/taste LLM judge** that replaces `llmEvaluator`'s 5
