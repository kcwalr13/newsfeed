...
- **Round 6 (LLM provider abstraction + go free-tier, 2026-06-15): 7 NEW items, all TODO.** Plan:
  ...
  **Progress: 6/7 DONE (R6-1…R6-6; R6-5 code done, live acceptance pending Kyle's Vercel flip) · 0 IN-PROGRESS · 1 TODO (R6-7 optional phase-2 batching). Last resume point: R6-7.**
...
- **R6-6 DONE** — docs + ops.
  ...
  - Gate: `tsc` clean · `build` EXIT=0 (docs-only; lint unaffected). Commit: `pending` (backfill in R6-7).
- RESUME AT: **R6-7** (optional phase-2 batching — likely **DEFERRED** per design "the round works without
  it"; the round is functionally complete with R6-1…R6-6).
